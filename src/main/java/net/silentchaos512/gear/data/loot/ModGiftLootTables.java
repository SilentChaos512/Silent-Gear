...
public class ModGiftLootTables implements LootTableSubProvider {
    ...
    public void generate(BiConsumer<ResourceKey<LootTable>, LootTable.Builder> biConsumer) {
        biConsumer.accept(SgItems.BLUEPRINT_PACKAGE.get().getDefaultLootTable(), LootTable.lootTable()
                ...
                .withPool(LootPool.lootPool()
                        .add(LootItem.lootTableItem(GearItemSets.KNIFE.blueprint()))
                )
                .withPool(LootPool.lootPool()
                        .add(LootItem.lootTableItem(GearItemSets.SWORD.blueprint())
                                .setWeight(11))
                        .add(LootItem.lootTableItem(GearItemSets.KATANA.blueprint())
                                .setWeight(5))
                        .add(LootItem.lootTableItem(GearItemSets.MACHETE.blueprint())
                                .setWeight(7))
                        .add(LootItem.lootTableItem(GearItemSets.SPEAR.blueprint())
                                .setWeight(8))
                )
                .withPool(LootPool.lootPool()
                        .add(LootItem.lootTableItem(GearItemSets.SHIELD.blueprint()))
                )
        );
    }
}
