    ...
    public static void onLivingHurt(LivingHurtEvent event) {
        if (event.getSource().isMagicDamage()) {
            ...
            if (event.getEntityLiving() instanceof PlayerEntity) {
                ...
                ((PlayerEntity) event.getEntityLiving()).inventory.damageArmor(event.getAmount());
            }
...
//            if (toolClass == null) toolClass = "";
    ...
    public static void onGearCrafted(PlayerEvent.ItemCraftedEvent event) {
        ...
        if (event.getPlayer() instanceof ServerPlayerEntity) {
            ...
            // Crude tool
            if (GearData.hasPart(result, PartConst.ROUGH_ROD) || GearData.hasPart(result, PartType.ROD, p -> p.containsMaterial(Const.Materials.WOOD_ROUGH))) {
