...
public class FragmentModel extends LayeredModel<FragmentModel> {
    ...
    public IBakedModel bake(List<MaterialLayer> layers,
        ...
        for (int i = 0; i < layers.size(); i++) {
            ...
            TextureAtlasSprite texture = spriteGetter.apply(new Material(PlayerContainer.LOCATION_BLOCKS_TEXTURE, layer.getTexture(GearType.FRAGMENT, 0)));
    ...
    private void buildFakeModel(Function<Material, TextureAtlasSprite> spriteGetter, ImmutableList.Builder<BakedQuad> builder, TransformationMatrix rotation, IMaterial material) {
        ...
        if (model != null) {
            ...
            if (exampleMain != null) {
                builder.addAll(getQuadsForSprite(0, spriteGetter.apply(new Material(PlayerContainer.LOCATION_BLOCKS_TEXTURE, exampleMain.getTexture(GearType.FRAGMENT, 0))), rotation, exampleMain.getColor()));
            }
        }
    }
