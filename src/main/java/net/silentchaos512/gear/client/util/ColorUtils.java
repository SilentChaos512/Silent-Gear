...
public class ColorUtils {
    public static int getBlendedColor(CompoundPartItem item, Collection<? extends IMaterialInstance> materials) {
        ...
        for (IMaterialInstance mat : materials) {
            ...
            if (model != null) {
                int color = model.getLayers(item.getGearType(), item.getPartType()).getPrimaryColor();
                int r = (color >> 16) & 0xFF;
                int g = (color >> 8) & 0xFF;
                int b = color & 0xFF;
