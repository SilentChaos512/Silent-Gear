...
public class GearFishingHookRenderer extends EntityRenderer<GearFishingHook> {
    private static final ResourceLocation TEXTURE_LOCATION = new ResourceLocation("textures/entity/fishing_hook.png");
    private static final RenderType RENDER_TYPE = RenderType.entityCutout(TEXTURE_LOCATION);
    private static final double VIEW_BOBBING_SCALE = 960.0D;

    public GearFishingHookRenderer(EntityRendererProvider.Context p_174117_) {
        super(p_174117_);
    }

    @Override
    public void render(GearFishingHook p_114705_, float p_114706_, float p_114707_, PoseStack p_114708_, MultiBufferSource p_114709_, int p_114710_) {
        // FIXME: This code is never called. Game uses the FishingHookRenderer for some reason. Seems the client thinks the bobber is the vanilla entity.
        // Same as super, but checks for gear fishing rod items so the fishing line renders on the correct side
        ...
        if (player != null) {
            ...
            if (!(itemstack.getItem() instanceof GearFishingRodItem)) {
                i = -i;
            }
